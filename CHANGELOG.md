...
2026-04-15 - Strict RBAC Enforcement Across University Dashboard - Completely secured all Admin-only dashboard pages (Events list, Single Event Details, Explore Market, and the Root Dashboard) by adding explicit role checks. Normal 'MEMBER' users will now be structurally redirected directly to their Scanner tool if they attempt to access any of these administrative pages via direct URL, fully eliminating the possibility of accessing administrative or global statistics.
2026-04-15 - Created Dedicated Simple Dashboard For MEMBER Role - Replaced the complex admin-level dashboard with a tailored scanner representative dashboard for MEMBER accounts. Successfully locked down the Analytics tab and simplified the sidebar so standard members now only see their specific 4 tools: Simple Dashboard, Scanner, Scanned Leads, and University Documents.
2026-04-15 - Implement Team Mode metrics and leads sharing for MEMBER roles - Migrated the simple scanner dashboard and leads page from individual tracking to Team Mode, so members now see the aggregate total of all scan data collected by their university instead of just their personally scanned lists.
2026-04-15 - Add station selector and Mobile layout - Added a station selection toggle on the Usher PIN Login page to let staff choose between Check-in Station and Red Points Help Desk. Additionally enhanced the admin scan page layout by using flex-col-reverse on mobile devices so the camera view takes top precedence over the sidebar stats.
